...
**Queue pillar composition check (prevents cross-burst pillar blocking):** Before writing a post for any pillar, count how many files for that pillar are ALREADY in the X queue — INCLUDING any files created earlier in the SAME session (they are written to disk immediately and count toward the queue). If any pillar represents ≥30% of the current X queue, that pillar is OVERACCUMULATED — skip it and substitute with the most under-represented pillar instead. **Intra-session re-check rule (added S1679):** When creating 2+ posts in a single session, re-run the filesystem pillar count after EACH file is written — do NOT rely on mental arithmetic from the session-start composition. A session that starts with P3=1/5=20% (safe), writes a P3 post → queue becomes P3=2/6=33% (blocked). The second file check MUST use the updated count (P3=2/6=33%), not the cached start count (P3=1/5=20%). Evidence: S1678 wrote P3 post when P3=1/6=17% (safe → became 2/6=25%), then wrote a second P3 post without re-checking → P3=2/5=25%→2/6=33%=BLOCKED. State file retrospective: "Should have checked post-addition P3 composition before creating the second P3 post." Root cause: pre-file check ran against cached composition; filesystem count not re-run between the two files created in the same session. Fix: After each content file is written (Write tool returns), run `find agent/outputs/x -name "*.txt" -type f | wc -l` and recount per-pillar before deciding the next file's pillar. This check is SEPARATE from the burst distribution check (which tracks burst-level balance) — it prevents filling the drain queue with one pillar that then blocks the NEXT burst. **How to run:** `find agent/outputs/x -name "*.txt" | xargs grep -l "keyword"` (use 2-3 key terms per pillar). Or track pillar counts in the state file after each post. **Threshold:** ≥30% = skip (e.g., if X queue = 10, any pillar with ≥3 files is overaccumulated). **Substitution rule:** When a burst slot mandates pillar X but X is overaccumulated in queue, substitute with the **most under-represented safe pillar** (lowest queue %). Do NOT pick the "next pillar in the slot table" by sequence — that's arbitrary. Instead: (1) check queue composition for all pillars, (2) find the safe pillar (< 30%) with lowest queue %, (3) use tiebreak P1 > P3 > P4 > P2 by owner expertise depth. If ALL content pillars are overaccumulated (≥30%), write a BIP post (BIP hooks always available). Document the substitution in the state file burst block (e.g., "Post 2: P4 BLOCKED in queue (31%) → P1 substitution (P1=8%, lowest safe)"). **Dual-pillar block case (B93 evidence):** When two mandatory early slots (e.g., P4 at post 2 AND P3 at post 4) are simultaneously overaccumulated, apply substitution for each blocked slot independently using the same most-under-represented rule. The unblocked mandatory slots (e.g., P2 at post 3) still fire as normal. Evidence: B93 started with P3=31% and P4=31% in queue simultaneously after B92 drained. State file correctly used "substitute P1 at Post 2 slot" based on P1=8% (lowest safe). Previous wording "NEXT pillar in slot table" was ambiguous — for P4-at-post-2, "next" implied P2 (post 3 slot), which is incorrect. Most-under-represented is the correct interpretation. **Reset:** Once the queue drains enough to bring the overaccumulated pillar below 30%, the block clears and the next burst can resume normal slot assignments. **Write-time labeling rule (prevents false BLOCKED labels):** When updating the state file's queue pillar composition section, ONLY label a pillar as "QUEUE-BLOCKED" when its queue percentage is ≥30%. Do NOT label at 25-29% — those percentages are safe. Incorrect BLOCKED labels in the state file propagate across sessions, causing sessions to skip a pillar that is actually safe to write. Evidence: S1578 and S1579 labeled P3 as "QUEUE-BLOCKED" when it was at 25% (4/16=25%), below the ≥30% threshold. S1580 retrospective caught the error — P3 at 25% was safe to write. Root cause: no explicit write-time constraint, so agents applied the 30% check at read time but wrote labels inconsistently. This rule adds the same precision to state file writes. **Sustained dual-pillar block: P1 overaccumulation guard (added S1469):** When P3 AND P4 are simultaneously queue-blocked for multiple consecutive bursts, the "most under-represented" rule repeatedly selects P1 as substitute because BIP has the only lower queue %. After 2+ substitutions, P1 exceeds its burst target (25%) while P3/P4 remain blocked — but P1 still "wins" the most-under-represented check relative to blocked pillars, causing further P1 overaccumulation. **Rule: Before selecting P1 as a substitution, check P1's current burst %:** (1) If P1 burst% < 25% (under target) → substitute P1 as normal. (2) If P1 burst% ≥ 25% (at/above target) → P1 is already served; substitute with BIP instead. BIP hooks always available (session count, PR count, follower count, burst number). Do NOT let P1 absorb a 3rd substitution when it has already exceeded its burst target. Evidence: B94 P1=40% (absorbed 4 substitutions — posts 2, 5, 8, 10 — while P3=0% and P4=0%); B95 P1=33% after post 4 (absorbed 2 substitutions by post 4, already above 25% target). Root cause: the "most under-represented" check uses queue % (P1=8%), not burst %. After absorbing substitutions, P1 burst% rises to 33-40% while P1 queue% stays low — the check never detects the burst-level overaccumulation. This rule adds the missing burst-% gate to the substitution decision. Evidence: B88 P4=10% (queue P4 overaccum discovered mid-burst), B89 P4=0% (entire burst blocked on P4 — first zero-P4 burst), B90 Post 2 P4 still blocked (P4=6/12=50% in queue). Root cause: no pre-post queue composition check. The burst-distribution-pre-check only looks at the current burst's pillar counts, not the queue's composition. A burst can have perfect P4 balance (20%) while the queue's P4 concentration blocks the NEXT burst entirely. This rule adds the missing queue-level check.

**Starvation recovery threshold (stricter pre-burst gate for chronically blocked pillars):** The standard pre-burst threshold (≥30%) can still allow a burst to start when a pillar is at 25-29% — and then the per-post check at the mandatory slot (e.g., P4 at Post 2, fired 1 session later) finds the pillar has risen above 30% because non-P4 files drained in between. This causes 0% representation for a second consecutive burst. **Rule: If a pillar shows ≤10% in the preceding burst (1 or fewer posts in a 10-post burst, due to queue-blocking or substitution), apply a STRICTER threshold of 20% for that pillar at pre-burst check.** Only start the burst when that pillar is below 20% in queue — not just 30%. The extra 10pp buffer accounts for queue composition shift between burst start (Post 1) and mandatory slot fire (Post 2, a session later). **Apply this stricter threshold only to the starved pillar** — other pillars still use the standard ≥30% gate. **Reset:** Once the starved pillar reaches ≥2 posts (≥20%) in a burst, starvation recovery threshold reverts to standard 30% for the next burst. Evidence (updated S2035): B129 P4=0% + B130 P4=0% (original evidence). B152 P4=10% (1 post) → B153 P4 still blocked (gate didn't fire at trigger=0%). B159 P4=10%↓ → B160 P4=10%↓ (identical oscillation). The =0% trigger was too strict — a burst where P4 got exactly 1 post (10%) still leaves enough P4 in queue to block the next burst's mandatory post-2 slot. Lowering trigger to ≤10% catches these near-zero bursts. **6 confirmed data points across 4 burst pairs** (B129-B130, B152-B153, B159-B160) support the ≤10% threshold. S1784 identified the timing gap; S2034 confirmed the trigger adjustment in publishing skill; S2035 added parallel rule to CLAUDE.md.

**BIP displacement flag (fixes 5-burst BIP=20% ceiling):** After writing burst post 5, check: was P1=0 before post 5? If YES (P1 mandate just fired for the first time this burst), set `displacement_flag: TRUE` in the state file burst block. At post 6: if `displacement_flag: TRUE` AND BIP=1 (only 1 BIP post so far), write BIP at post 6 — NOT P2 secondary slot. BIP wins the post-6 slot when displacement occurred. After writing BIP at post 6, set `displacement_flag: BIP-MIDPOINT-FIRED` (NOT `RESOLVED` — see critical fix below). If `displacement_flag: FALSE` or P1 already had ≥1 post before post 5, apply the standard P2 secondary slot rule at post 6. **Why this matters:** 5 consecutive bursts (B87-B91) ended at BIP=20% (below 25% target) due to this exact failure mode — P2 secondary slot fired at post 6 before the BIP displacement check could run. The publishing skill documents "BIP wins post-6 in displacement case" but the agent cannot detect displacement at execution time without a state variable. The `displacement_flag` closes this detection gap. **State file format** (add to burst block after post 5 is written): `- displacement_flag: TRUE (P1 mandate fired at post 5, BIP midpoint displaced — BIP must get post 6 over P2 secondary slot)`. **CRITICAL FIX (added S2137 — B176 recurrence):** After BIP fires at post 6 via displacement, the flag MUST be set to `BIP-MIDPOINT-FIRED` (not `RESOLVED`). Rationale: `RESOLVED` erases the displacement context before the back-half check at post 7-8. The back-half BIP check (BIP≤2 absolute) fires anyway because the agent cannot distinguish "displacement burst" from "standard burst" when the flag reads `RESOLVED`. This caused B176 P2=10%↓ — a BIP at post 8 displaced P2's back-half slot. With `BIP-MIDPOINT-FIRED`: at post 7-8, check if `displacement_flag: BIP-MIDPOINT-FIRED` — if YES, mark the BIP back-half check as SATISFIED (skip it), freeing the slot for P3/P4/P1/P2 back-half checks. At burst end after all back-half checks complete, update `displacement_flag: RESOLVED`. **Flag lifecycle:** TRUE (post 5 written, P1=0) → BIP-MIDPOINT-FIRED (post 6 BIP written via displacement) → RESOLVED (burst complete, all back-half checks done). Never jump from TRUE→RESOLVED or BIP-MIDPOINT-FIRED→RESOLVED before back-half checks run. Evidence: B69-B70 (original detection, S1254), B176 (recurrence, S2136) — same root cause: flag=RESOLVED before back-half check → BIP≤2 fires → P2 displaced. 2 confirmed instances meeting CLAUDE.md quality gate (2+ occurrences, clear mechanism, actionable fix). Evidence: B87=20%↓, B88=30%✓ (no displacement that burst), B89=20%↓, B90=20%↓, B91=20%↓ — 4 of 5 bursts failed due to this pattern. B88 was the only burst where post 5 was NOT P1 (no displacement) — confirming displacement as the root cause. Implementing the flag costs 1 line in the state file per burst; fixing BIP=20%→25-30% is the highest-ROI improvement available.

**Back-half check burst-% gate (prevents overcorrection when thread already hit pillar target):** All back-half checks (P3, P4, P1, P2) fire on absolute count (P3=1, P1=1, P4<15%) without verifying the pillar isn't already ≥20% of the current burst. When a thread occupies a burst slot for pillar X, that post counts as +1 pillar X. A subsequent back-half check then writes another +1 → pillar X = 3/N posts = 30%+ (overcorrection). **Rule: Before firing any back-half check, verify the target pillar is < 20% of current burst posts. If already ≥20%, skip that check and proceed to the next-priority pillar.** This prevents the thread+back-half-check double-count pattern. Evidence: 4 confirmed instances (B223, B227, B229, B230) where a thread at post 7 + back-half check at post 8 pushed a pillar to 30%+ overaccumulation. Quality gate met: 4 instances, clear mechanism (thread double-count), actionable fix (burst-% gate check). Added S2600. Mirrors rule in publishing skill back-half check sections. **Confirmed effective: B233 (S2623 audit) — all 3 back-half checks (P3, P4, P2) fired with gate active, zero overcorrection, 5th perfect 20% burst. B234 state shows P1=30%/P3=30% but this is a state file accounting error — thread sub-posts counted as multiple pillar contributions vs. correct 1 queue file = 1 burst post = 1 pillar contribution.** **State file counting rule: 1 queue file = 1 burst post = 1 pillar contribution regardless of how many tweets the thread contains. A thread with 4 sub-posts = +1 P1, not +4 P1. When computing pillar distributions for burst planning, count files created, not tweet sub-counts.**
...
- **If any queue = 11-12 (look-ahead zone):** Max 1 X file total — this includes BOTH content posts AND reply files. **Replies count as X queue files.** The session prompt CONTENT TARGET ("include at least 1 reply") is subordinate to queue rules — at X=11-12, do NOT create a reply if any X content has already been created this session. Evidence: S1561 created 1 BIP + 1 reply at X=12 → X=14 violation. Root cause: "include at least 1 reply" content target competed with "max 1 X piece" look-ahead rule. Same failure mode as S1148 (burst plan obligations vs pre-file-creation check). **Intentionally invalid reply files are NOT a workaround.** A reply file with an invalid REPLY_TO (URL format, @handle, or placeholder) still occupies a queue slot until the pipeline moves it to `skipped/`. Creating an invalid reply "dummy" file at X=11-12 after content has been created is the same violation as creating a valid reply — both push the queue above the threshold. Evidence: S1689 created reply-20260708-001.txt with invalid REPLY_TO at X=12 (after 1 BIP post) → filesystem X=14 while state wrote X=13. Root cause: agent tried to satisfy "include at least 1 reply" content target using an invalid dummy reply file. Rule: do NOT create any reply file (valid OR invalid) in the look-ahead zone when X content was already created this session. **Exception: if BS queue < 8, write a Bluesky-only post instead (no X file) — X queue stays at 11-12, BS capacity is recovered.** Creating 2 files at 11 → queue hits 13 → next session immediately blocked. Evidence: S209 created 2 files (queue 11→13) → S210 blocked. The productive pattern is 1 file/session at 11-12, not 2 files + 1 blocked session. Evidence for BS-only exception: S479 (2026-04-09) applied this correctly (X=12, BS=7 → 1 BS-only companion created, BS=7→8, X unchanged). See publishing skill for full exception details. **BIP preference in look-ahead zone:** When exactly 1 X post is allowed (X=11-12), prefer a BIP post or thread over topic news. Evidence: B23 BIP=17%, B24 BIP=18%, B26 BIP=20.2% — all below 25% target across 4 consecutive bursts. Root cause: agent defaults to news hooks when limited to 1 piece. BIP posts serve multiple goals (milestone tracking, authenticity, engagement) and fall below target precisely because look-ahead sessions default to news. Threads also count as 1 file but generate 40-60% more reach. Apply: if current burst BIP% < 25%, always choose BIP or thread over topic news in look-ahead zone. **If BIP ≥ 25% (already on target), choose the most under-target pillar** — the one with the largest gap from its target (check BIP checklist item 9 for current burst distribution). Priority: most-under-target pillar wins. Tiebreak: P1 > P3 > P4 > P2 (by owner expertise depth). Evidence: B54 post 10 scenario — BIP=33% (on target), P1=11% and P2=11% (both 9-14% below target). Without this rule, agent ambiguity → defaults to first available research hook rather than most-needed pillar. With rule: P1 wins tiebreak (deepest expertise) → deterministic selection. **QUEUE BIP% GATE (added S2494 — B222 startup delay):** Before choosing BIP in look-ahead zone, ALSO check queue BIP%. If queue BIP% ≥ 25% (even if below the 30% pre-burst gate), skip BIP preference and choose the most under-represented safe pillar instead. Reason: adding BIP when queue BIP is already 25-29% risks pushing it over the 30% threshold that blocks next burst start. The burst BIP% check (above) and queue BIP% check are INDEPENDENT — both must pass before choosing BIP in look-ahead zone. Example: after B221 (BIP=30%✓ burst), queue has 3 BIP files. At X=9, BIP=3/9=33% (above 30% — already blocked). At X=12, BIP=3/12=25% — passes burst-BIP check but queue BIP is exactly at the 25% gate. Adding 1 BIP at X=12 → X=13: 4/13=31% → blocked next session. Skip BIP; choose under-represented pillar. Evidence: S2491 tweet-150 (BIP look-ahead post) pushed queue BIP from 3/9=33% to 4/9=44% → blocked B222 for 2+ sessions. The burst BIP% was below 25% (BIP was 30% final but S2491 was a look-ahead AFTER burst completed, checking the most recent burst's target). Root cause: no queue BIP% check in look-ahead decision — only burst BIP% was checked. 2 confirmed instances (S2491 + B221 look-ahead pattern) meeting quality gate. **Practical check:** Before writing BIP in look-ahead zone, run: `find agent/outputs/x -name "*.txt" | wc -l` (total) and count BIP files. If BIP/total ≥ 25%, write a non-BIP post for the pillar most underrepresented in queue instead.
...
- **BS companion limit during burst fill (X ≤ 10, all queues normal):** Even when X queue allows 2 posts, BS companion creation is restricted. Rule: BS companions must not push BS queue above 6. If BS_start >= 7, create ZERO BS companions this session regardless of X capacity. Arithmetic: BS_start + companions_this_session ≤ 6 must hold. Evidence: S1574 (X=6, BS=7) created 2 BS companions → BS=9 (near-throttle violation, B109 burst start); S757/B26 same pattern. Root cause: CLAUDE.md queue rules only specified BS=8-9 as blocked, not the pre-near-throttle companion limit. Full rule detail in publishing skill "BS companion limit during burst fill" section.
...
3. **CLAUDE.md improvement** — Identify a recurring inefficiency in agent behavior. Propose a protocol change. Document reasoning. Update the file. **Quality gate (prevents manufactured edits):** A valid CLAUDE.md improvement requires: (a) the inefficiency has occurred at least 2 times (with session references), (b) a clear mechanism explaining why the current rules don't prevent it, and (c) the proposed rule is actionable without ambiguity. If you cannot meet all three criteria, this option is NOT eligible — mark as exhausted and proceed to Tier 2. Evidence: sessions can always find *something* to tweak in CLAUDE.md, but marginal changes (e.g., adding a sentence that restates an existing rule) produce zero behavioral improvement and waste CI minutes. The quality gate distinguishes genuine protocol fixes (3 recurring instances → new rule) from rationalized edits that exist only to justify a PR. **Burst final-post deferral rule (evidence-based addition):** When a burst reaches N-1/N posts (1 post remaining) and that final post is blocked by queue overaccumulation for 3+ consecutive sessions, the burst is considered COMPLETE at N-1 posts. Begin B(N+1) planning immediately. Rationale: indefinite "post deferred" state bloats the state file with repeated re-evaluations and consumes turns checking if the blocked pillar has drained. Queue will naturally drain; the next burst can fill the gap. Exception: if the final post is a mandatory BIP front-load for the current burst (not yet written), wait until queue allows BIP (BIP is never blocked by pillar overaccumulation — queue BIP% is always safe by definition since BIP is rarely overaccumulated). Evidence: B93 Post 10 deferred from S1451 onward (P3 blocked at 31%). B92 Post 10 was similarly deferred but completed before 3 sessions elapsed. If B92 had not completed, it should have closed at 9/10 after 3 blocked sessions.
