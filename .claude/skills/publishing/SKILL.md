...
| Posts 7+ | Any (back-half checks apply) | See checklist item 9/10 | Back-half enforcement at posts 7-8. Thread check: if threads_this_burst = 0, write thread at post 7 or 8. |

**Back-half check priority (posts 7-8, when multiple fire):** Thread (if 0 this burst) > BIP > P3 > P4 > P1 > P2
...
**P3 back-half check rule (burst post 7-8):** P3 appears at post 4 (first-4-posts mandate), then gets no additional slots as news hooks dominate the burst back half. Pattern: P3 consistently ends at exactly 1 post per burst = 10-13% of a 8-10 post burst, below the 20-25% target. Root cause: no enforcement after post 4. **Rule: At burst post 7-8 (70-80% point), if P3 = 1 post total (absolute count) AND P3 < 20% of current burst posts, write a P3 post before continuing other pillars.** The burst-% gate (P3 < 20%) prevents overcorrection when P3 already reached target via a thread earlier in the burst — thread counts as +1 P3, and adding a back-half post would push P3 to 3/N (30%+). For a 10-post burst: if P3=1 at post 7 (10%), write P3 as post 8 → P3=2/8=25% ✓. **If P3=2/7=29% at post 7 (already above 20% threshold — e.g., post 4 mandate + thread), skip this check.** P3 back-half hooks: call center AI ROI, voice AI adoption, CX automation, Ender Turing domain data, agent attrition/performance. Evidence: B49 P3=10% (1/10), B50 P3=13% (1/8) — both bursts had P3 appear at post 4 but received no second slot. Identical pattern to P4 before back-half check was added. Same intervention. **Overcorrection evidence: B223, B227, B229, B230 — 4 bursts where P3 reached 30%+ because back-half check fired when P3 was already ≥20% via thread+mandate combination. Quality gate met (4 instances, S2600). Confirmed effective: B233 (S2600 rule) — P3 back-half fired at post 8 (P3=1/7=14%<20% → fired → P3=2/8=25%✓). B233 = 5th perfect burst, first production case where burst-% gate ran correctly for all 3 back-half checks (P3, P4, P2). B234 state shows P3=30% but this is likely a state file accounting error (thread sub-posts counted as multiple pillar posts vs. 1 queue file = 1 burst post). See S2623 audit.** **State file counting rule: Each queue FILE = 1 burst post = 1 pillar contribution. A 4-tweet thread file = 1 pillar contribution, not 4. When updating state file pillar distribution, count files in `agent/outputs/x/`, not tweet sub-counts.** Target: P3 = 20-25% per burst.

**P4 back-half check rule (burst post 7-8):** P4 passes the midpoint threshold (≥15%) but consistently finishes below target in the back half of bursts. Root cause: P4 appears early (satisfying the first-3-posts rule), looks fine at midpoint (1/5=20%), then gets zero back-half slots as news hooks dominate posts 6-10. **Rule: At burst post 7-8 (70-80% point), if P4 < 15% of total burst posts so far AND P4 < 20% of current burst posts, write a P4 post before continuing other pillars.** The burst-% gate (P4 < 20%) prevents overcorrection when P4 already reached target via a thread. For a 10-post burst: if P4=1 at post 7 (14%), write P4 as post 8. **If P4=2/7=29% at post 7 (already at/above 20% threshold), skip this check.** This is the same back-half enforcement applied to BIP (added S1026, confirmed effective in B45). Evidence: B45 P4=10% (1/10) — P4 was 1/5=20% at midpoint (no flag), then 1/7=14% at post 7 (no rule → no action). Final: 10%. B26=14%, B27=8%, B29=15% — pattern: P4 fine at midpoint, below target at burst end. The back-half check closes this gap. **Confirmed effective: B50 P4=14% at post 8 (1/7) → back-half check fired → P4 post written as post 8 → P4=2/8=25% ✓.** First confirmed production case of P4 back-half check firing. P4 back-half hooks: AI funding round, inference cost update, LLM economics analysis, startup/VC trend piece.
...
**P2 back-half check rule (burst post 7-8):** Retained as fallback for bursts where the post-6 P2 secondary slot was consumed by other urgent content. **Rule: At burst post 7-8, if P2 < 15% of total burst posts so far AND P2 secondary slot (post 6) was not used AND P2 ≤ 1 post total (absolute count) AND P2 < 20% of current burst posts, write a P2 post before continuing other pillars.** The burst-% gate (P2 < 20%) prevents overcorrection. Priority in slot conflict: P2 remains lowest (BIP > P3 > P4 > P1 > P2) — this back-half rule is a safety net, not the primary enforcement. The post-6 secondary slot rule is primary. **Critical: if P2 ≥ 2 posts by post 7-8 (e.g., post 3 primary + post 7 via BIP-displacement path), the back-half check must NOT fire — P2 already has its guaranteed 2 posts (20% of a 10-post burst). Firing when P2=2 results in P2=30%+ (over target). Evidence: B86 P2=30% (3 posts: primary at post 3 + secondary at post 7 [BIP displaced post 6] + possible back-half) — the missing absolute-count guard let P2 overaccumulate past the 25% ceiling. Updated S1389.** If P2=2/7=29% at post 7 (above 20% threshold), skip this check regardless of the absolute-count condition.

**P1 back-half check rule (burst post 7-8):** P1 satisfies the first-5-posts mandate (post 5) but has no back-half enforcement. Pattern: P1 consistently ends at exactly 1 post per burst = 10-11% of a 10-post burst, below the 20-25% target. Root cause: the back-half slots (posts 7-10) are consumed by BIP (≤2 absolute), P3 (=1 absolute), P4 (<15%), and P2 (<15%) checks — leaving no slot for a 2nd P1 post. **Rule: At burst post 7-8 (70-80% point), if P1 = 1 post total (absolute count) AND P1 < 20% of current burst posts, write a P1 post before continuing other pillars.** The burst-% gate (P1 < 20%) prevents overcorrection when P1 already reached target via a thread earlier in the burst. If P1=2/7=29% at post 7 (above 20% threshold), skip this check. For a 10-post burst: if P1=1 at post 7, write P1 as post 8 → P1=2/8=25% ✓. P1 back-half hooks: autonomous agent PR milestones, agentic workflow architecture patterns, AI agent governance, multi-agent coordination, this repo's own session data, agent failure modes. Evidence: B58 P1=10% (1/10), B59 P1=11% (1/9 with last slot P2) — two consecutive 10-post bursts with P1 below 20% target. Same intervention logic as P3 (confirmed B51+), P4 (confirmed B50+), P2 (confirmed B58+). The complete back-half enforcement system now covers all pillars. **Confirmed effective: B61 P1=20%✓, B62 P1=20%✓ — two consecutive bursts hitting P1 target after rule was added (S1144).** Note: P1 is 4th in the back-half slot conflict priority (BIP > P3 > P4 > P1 > P2) — P1 fires after BIP/P3/P4 checks resolve, but before P2. Rationale: P2 reliably hits 20% via first-3-posts mandate alone (B58 P2=20%✓, B59 P2=20%✓, B60 P2=20%✓); P1 cannot reach 20% without this slot (B58 P1=10%↓, B59 P1=10%↓, B60 P1=10%↓ — 3 consecutive bursts). Swapping P1↔P2 costs P2 one back-half slot but gains P1 reliable 20% representation. **Known cost (updated S1160):** After P1/P2 priority swap (S1144), P2 dropped to 10% in B61 and B62 — two consecutive bursts below target. Fix: P2 secondary slot rule at post 6 (see above) secures P2 BEFORE back-half contested territory, eliminating the zero-sum dependency. Updated S1144 (swap) + S1160 (post-6 fix). Target: P1 = 20-25% per burst.
...
**Starvation recovery threshold (stricter pre-burst gate for chronically blocked pillars):** The standard pre-burst threshold (≥30%) can still allow a burst to start when a pillar is at 25-29% — and then the per-post check at the mandatory slot (e.g., P4 at Post 2, fired 1 session later) finds the pillar has risen above 30% because non-P4 files drained in between. This causes 0% representation for a second consecutive burst. **Rule: If a pillar shows ≤10% in the preceding burst (1 or fewer posts in a 10-post burst, due to queue-blocking or substitution), apply a STRICTER threshold of 20% for that pillar at pre-burst check.** Only start the burst when that pillar is below 20% in queue — not just 30%. The extra 10pp buffer accounts for queue composition shift between burst start (Post 1) and mandatory slot fire (Post 2, a session later). **Example:** If P4≤10% in B130 AND P4 files > 0 in queue at B131 pre-burst check: require P4 < 20% before starting (not just < 30%). At X=12 queue with 3 P4 files: 3/12=25% (fails standard 30% check anyway). At X=8 queue with 2 P4 files: 2/8=25% (passes standard check but fails starvation threshold). Wait until P4=1/X < 20% — e.g., queue of 6+ non-P4 files so 1/7=14% or drain until only 1 P4 file remains and total queue ≥ 6. **Apply this stricter threshold only to the starved pillar** — other pillars still use the standard ≥30% gate. Evidence (updated S2034): B129 P4=0% + B130 P4=0% (original evidence). Additional evidence: B152 P4=10% (1 post, scraping minimum) → B153 P4 still blocked (gate didn't fire at trigger=0%). B159 P4=10%↓ → B160 P4=10%↓ (identical oscillation). The =0% trigger was too strict — a burst where P4 got exactly 1 post (10%) still leaves enough P4 in queue to block the next burst's mandatory post-2 slot. Lowering trigger to ≤10% catches these near-zero bursts. **6 confirmed data points across 4 burst pairs** (B129-B130, B152-B153, B159-B160) support the ≤10% threshold change. S1784 identified the timing gap; S2034 confirmed the trigger adjustment. Reset: once P4 ≥ 2 posts in a burst (≥20% of 10-post burst), starvation recovery threshold reverts to standard 30% for the next burst.
...
5. **Thread check**: Have you created 2+ threads this week? If not, make one now. **Thread pillar diversity**: If the burst already has 1 thread on pillar X, the next thread must be a different pillar. Two threads on the same pillar in one burst = automatic overweighting. Evidence: B41 both threads were P3 → P3=30% (over target). **Thread back-half enforcement (burst post 7-8):** If thread count = 0 for the current burst AND post 7-8 window has been reached, write a thread (4-6 posts, `---` separator) before any single-post content. A thread counts as 1 queue file but generates 40-60% more reach than a single post. Evidence: B134=0 threads (10 posts), B135=0 threads (8 posts) — 2 consecutive bursts with no thread despite the 2+ threads/week target. Root cause: no burst-level enforcement; the "2 threads/week" guideline is aspirational without a back-half check. Rule: **At post 7-8, if threads_this_burst = 0, write a thread as post 7 or 8.** Thread pillar: use the most under-represented safe pillar (same most-under-represented rule as pillar substitution). Thread hooks always available: autonomous agent milestone (P1), call center AI ROI case (P3), marketing automation measurement (P2), inference economics (P4). Track in state file burst block: `- threads_this_burst: N`.
...
9. **Pillar diversity (per-burst enforcement)**: No single pillar > 50% of posts in the current burst. Count the burst total, not just last 5. If P1 already ≥50% of this burst: write P2/P3/P4 next. **If P1 = 0 after completing burst post 4: P1 MUST be the very next post (post 5). Do not let P1 slip to post 6+.** (The mandate says "first 5 posts" — P1 at post 6+ violates it. Check AFTER post 4, not post 5. Prior wording "P1 = 0 at burst post 5: MUST be post 6" was self-contradictory — post 6 is outside the first-5 window. Evidence: B50 P1 appeared at post 5 correctly.) See P1 first-5-posts mandate above. **If P2 already ≥25% before burst midpoint: skip P2 for next 2 posts** (see P2 mid-burst ceiling rule above). **If P3 < 20% at burst midpoint: run P3 proactive search immediately** (see Research Cadence above). **If P4 < 15% at burst midpoint: run P4 proactive search immediately** (P4 underweighted in B26 14%, B27 8%, B29 15% — same correction pattern as P3). **If P4 < 15% at burst post 7-8 AND P4 < 20% of current burst posts: write P4 post immediately** (see P4 back-half check rule — B45 P4=10% shows midpoint check insufficient; burst-% gate added S2600 to prevent overcorrection when thread already gave P4 ≥20%). **If P3 = 1 post total at burst post 7-8 AND P3 < 20% of current burst posts: write P3 post immediately** (see P3 back-half check rule — B49 P3=10%, B50 P3=13%; burst-% gate added S2600 — 4 overcorrection instances B223/B227/B229/B230 when thread+mandate pushed P3 to 30%+). **If P2 < 15% at burst post 7-8 AND P2 ≤ 1 post total AND P2 < 20% of current burst posts: write P2 post immediately** (see P2 back-half check rule — B51 P2=10%, slot conflict caused by BIP+P1+P4 back-half checks; updated S1389: absolute-count guard; burst-% gate added S2600). **If P1 = 1 post total at burst post 7-8 AND P1 < 20% of current burst posts: write P1 post immediately** (see P1 back-half check rule — B58 P1=10%, B59 P1=11%, B60 P1=10%, three consecutive bursts; P1 fires 4th in slot conflict after BIP/P3/P4, before P2; burst-% gate added S2600). **When multiple back-half checks fire simultaneously: use priority order BIP > P3 > P4 > P1 > P2** (see Back-half slot conflict resolution above — updated S1144: P1 promoted above P2 because P2 reliably hits 20% via first-3-posts mandate alone while P1 is structurally stuck at 10% when P2 holds slot priority; B52 evidence: P3 and BIP both fired at post 7, consuming slots needed by P4; B58-B60 evidence: BIP+P3+P4+P2 consumed all back-half slots across 3 consecutive bursts). Evidence: Burst 1 = 75% P1, Burst 3 = 58% P1. Week 21 P2 = 31% (chronic overaccumulation). P3 and P4 both underweight in recent bursts despite per-post checks. Burst-level tracking is required.
10. **BIP balance**: At least 25% of recent output. **BIP MUST be in the first 3 posts of every burst** (see BIP front-loading rule above). **If BIP < 25% at post 5 AND P1=0 (mandate fires instead): check `displacement_flag` in state file at post 6. If TRUE and BIP=1: write BIP at post 6 before P2 secondary slot** (see CLAUDE.md "BIP displacement flag" protocol for state variable tracking — B87-B91 all BIP=20% due to missing flag detection). After BIP fires at post 6 via displacement, **set `displacement_flag: BIP-MIDPOINT-FIRED`** (not RESOLVED — RESOLVED erases context before back-half check). **At burst post 7-8: FIRST check `displacement_flag` — if `BIP-MIDPOINT-FIRED`, skip the BIP≤2 back-half check entirely (BIP back-half SATISFIED by displacement). Then proceed to P3/P4/P1/P2 back-half checks.** Only if `displacement_flag` is absent or FALSE: apply the BIP≤2 absolute check. Evidence: B176 P2=10%↓ because `displacement_flag: RESOLVED` (set after post 6) was read at post 7-8 as "standard burst" → BIP≤2 check fired → BIP at post 8 → P2 lost slot. If `displacement_flag: BIP-MIDPOINT-FIRED` had been read instead, post 8 slot goes to P2 → P2=20%✓. (Added S2137, B176 recurrence.) **At burst post 7-8: if BIP ≤ 2 posts total (absolute count) AND displacement_flag is NOT BIP-MIDPOINT-FIRED, write BIP next** — do NOT use percentage here; 2/7=29% looks fine but projects to 2/10=20% if no more BIP is added. Use absolute count only. (B48 evidence: denominator blind spot caused 9th consecutive under-25% burst.) Evidence of structural displacement problem: B63, B64, B65 all showing BIP=20% (2/10) despite 3 enforcement rules — root cause is midpoint check never fires because P1 mandate always claims post 5.
